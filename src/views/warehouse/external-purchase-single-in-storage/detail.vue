<template>
  <div class="view-wrap external-purchase-in-storage-detail-page">
    ...
    <div class="section-title"><span class="section-bar" /><span>外购产品信息</span></div>
    <div class="table-box">
...
const RU_KU_API = '/ruKuPurchaseForeignProductOrder';
      ...
      this.$api({
        ...
        .then(res => {
          if (res && res.code === 200 && res.data) {
            ...
            const products = Array.isArray(d.productJson) ? d.productJson : [];
            this.detailMaterials = products.map(line => {
              const info = line.info || {};
              const rk = line.rkNum != null ? String(line.rkNum) : '';
              const num = line.num != null ? String(line.num) : '';
              return {
                lineId: line.id,
                foreignProductId: line.foreignProductId,
                code: info.productNo || '',
                name: info.title || '',
                spec: info.keyVals || '',
                purchaseQty: num,
    ...
    confirmInStorage() {
      ...
      for (let i = 0; i < this.detailMaterials.length; i++) {
        ...
        if (q === '' || isNaN(n) || n < 0 || !Number.isInteger(n)) {
          this.$message.warning(`请为「${row.name || '产品'}」填写有效的非负整数入库数量`);
          return;
        }
        const max = Number(row.purchaseQty);
        if (!isNaN(max) && n > max) {
          this.$message.warning(`「${row.name || '产品'}」入库数量不能大于采购数量 ${row.purchaseQty}`);
          return;
        }
        if (row.lineId == null || row.foreignProductId == null) {
          this.$message.warning('明细数据不完整');
          return;
        }
        productJson.push({
          id: String(row.lineId),
          foreignProductId: String(row.foreignProductId),
          rkNum: String(n)
        });
      }
