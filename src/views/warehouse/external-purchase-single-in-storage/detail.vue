<template>
  <div class="view-wrap external-purchase-in-storage-detail-page">
    ...
    <div class="base-info-grid">
      <div class="grid-item"><span class="label">外购请购单单号：</span><span class="value">{{ detailInfo.purchaseNo }}</span>
      </div>
      <div class="grid-item"><span class="label">下单时间：</span><span class="value">{{ detailInfo.submitTime }}</span>
      </div>
      <div class="grid-item"><span class="label">销售金额：</span><span class="value">{{ detailInfo.orderAmount }}</span>
      </div>
      <div class="grid-item"><span class="label">所属订单号</span><span class="value">{{ detailInfo.purchaseName }}</span>
      </div>
      <div class="grid-item">
    ...
    <div class="section-title"><span class="section-bar"></span><span>外购产品信息</span></div>
    <div class="table-box">
    ...
    <div v-else class="stored-section">
      ...
      <div class="table-box">
        <el-table :data="inStorageList" header-cell-class-name="table-header-cell">
          <el-table-column prop="inTime" label="入库单号" min-width="200" />
          <el-table-column prop="inTime" label="入库时间" min-width="200" />
          <el-table-column prop="inTime" label="质检报告" min-width="200" />
          <el-table-column prop="inPerson" label="质检备注" min-width="160" />
        </el-table>
      </div>
    </div>
  </div>
...
<script>
export default {
  name: 'ExternalPurchaseSingleInStorageDetail',
  data() {
    return {
      detailId: '',
      detailInfo: {
...
.external-purchase-in-storage-detail-page {
  background: #fff;
  border-radius: 8px;
  padding: 24px;
}
