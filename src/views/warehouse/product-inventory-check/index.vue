<template>
  <div class="view-wrap inventory-check-page">
    ...
    <!-- 新增盘点弹框 -->
    <el-dialog title="新增盘点" :visible.sync="addCheckDialogVisible" width="520px" :close-on-click-modal="false"
      ...
      <el-form ref="addCheckForm" :model="addCheckForm" :rules="addCheckRules" label-width="100px">
        ...
        <el-form-item label="盘点图片：" prop="attachmentUrl">
          <el-upload class="check-upload" list-type="picture-card" :action="uploadAction" name="file"
            :file-list="checkFileList" accept="image/*" :limit="1" :on-success="handleCheckUploadSuccess"
            :on-remove="handleCheckUploadRemove" :http-request="handleCheckUploadRequest">
    ...
    <!-- 盘点详情：从右向左抽屉，仅信息展示 -->
    <el-drawer title="盘点详情" :visible.sync="stocktakDetailVisible" direction="rtl" size="560px" :append-to-body="true"
      :close-on-click-modal="false" custom-class="stocktak-detail-drawer-wrap" @closed="onStocktakDetailClosed">
      <div class="stocktak-detail-drawer">
        <div class="stocktak-detail-info">
          ...
          <div v-if="stocktakDetail.url" class="info-row">
            <span class="label">盘点图片：</span>
            <div class="value">
              <el-image class="detail-thumb" :src="stocktakDetail.url" fit="contain"
...
const LIST_API = '/getProductStocktakList';
const ADD_API = '/addProductStocktak';
const DETAIL_API = '/getProductStocktakInfo';
...
export default {
  ...
  data() {
    return {
      ...
      addCheckForm: {
        checkName: '',
        remark: '',
        /** 上传接口返回的文件地址，提交 addProductStocktak 时用 */
        attachmentUrl: ''
      },
      addCheckRules: {
  ...
  methods: {
        ...
        .then(res => {
          if (res && res.code === 200 && res.data) {
            ...
            this.tableData = list.map(it => ({
              ...
              checkName: it.title || '',
              remark: it.cont || '',
              checkTime: (it.created_at || '').slice(0, 10)
            }));
    ...
    /** 与设备采购合同图片等一致：自定义上传至 UPLOAD_ROOT */
