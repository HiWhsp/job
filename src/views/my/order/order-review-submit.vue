...
export default {
  ...
  methods: {
    ...
    submit_pingjia() {
      ...
      if (this.upload_pic_list.length > this.upload_limit_number) {
        alertErr("最多上传" + this.upload_limit_number + "张图片");
        return;
      }

      const arr = [];
      this.upload_pic_list.forEach(item => {
        arr.push(item.response.data);
      })

      this.params.uploadedfile1 = arr.join("|");
