<template>
  <div class="purchase-detail-page">
    ...
    <div class="section-card">
      <div class="section-header">
        <span class="section-title">包装信息</span>
      </div>
      <div class="section-body">
        <el-table :data="detail.itemList" class="data-table" :row-class-name="tableRowClassName">
          <el-table-column prop="itemCode" label="包装编码" min-width="140" />
          <el-table-column prop="itemName" label="包装名称" min-width="140" />
...
<script>
export default {
  name: "ExternalPackagePurchaseDetail",
  data() {
    return {
      detail: {
        purchaseNo: "",
...
.purchase-detail-page { background: #fff; border-radius: 8px; padding: 24px; padding-bottom: 32px; }
.page-title { font-size: 18px; font-weight: 500; color: #333; margin-bottom: 24px; text-align: left; }
.section-card { margin-bottom: 24px; }
.section-header { display: flex; align-items: center; padding-bottom: 12px; margin-bottom: 20px; border-bottom: 1px solid #e5e5e5; }
.section-title { position: relative; padding-left: 15px; font-size: 16px; font-weight: 500; color: #2373c8;
  &::before { content: ""; position: absolute; left: 0; top: 2px; bottom: 2px; width: 4px; height: 20px; background: #2373c8; border-radius: 2px; }
}
.section-body .field-row { margin-bottom: 16px; }
.field-item { font-size: 14px; color: #606266; display: flex; align-items: center; flex-wrap: wrap; }
.field-label { min-width: 100px; text-align: left; color: #333; }
.field-value { color: #333; }
.data-table {
  ::v-deep .el-table__header th { background: #f5f7fa; color: #303133; font-weight: 500; }
  ::v-deep .el-table__body tr.row-even td { background: #fafafa; }
  ::v-deep .el-tag--plain { border: 1px solid #dcdfe6; background: #fff; color: #606266; }
}
.form-footer { margin-top: 32px; padding-top: 24px; border-top: 1px solid #f1f1f1; text-align: left;
  .el-button { min-width: 112px; height: 38px; padding: 10px 24px; }
  .el-button--primary { background: linear-gradient(90deg, #157de9 0%, #3697fd 100%) !important; border: none; }
  .el-button:not(.el-button--primary) { background: #fff; border-color: #dcdfe6; color: #606266; }
}
</style>
