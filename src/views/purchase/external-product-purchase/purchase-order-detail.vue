<template>
    <div class="view-wrap generate-order-page">
            ...
            <div class="meta-grid">
                <div class="row-item">
                    <div class="meta-item">
                        ...
                        <span class="meta-value">{{ detail.purchaseNo || "—" }}</span>
                    </div>
                    <div class="meta-item">
                        <span class="meta-label">采购单名称:</span>
                        <span class="meta-value">{{ form.purchaseName || "—" }}</span>
                    </div>
                    <div class="meta-item">
                        <span class="meta-label">状态:</span>
                        <span class="meta-value">
                            <el-tag :type="purchaseOrderStatusTagType(detail.orderStatus)" size="small">{{
                                purchaseOrderStatusLabel(detail.orderStatus) }}</el-tag>
                        </span>
                    </div>
                </div>
                <div class="row-item">
                    <div class="meta-item">
                        ...
                        <span class="meta-value">{{ isPayText(detail.isPay) }}</span>
                    </div>
                    <div class="meta-item">
                        <span class="meta-label">所属订单号:</span>
                        <span class="meta-value">{{ detail.staffOrderNo || "—" }}</span>
                    </div>
                    <div class="meta-item">
                        <span class="meta-label">订单时间:</span>
                        <span class="meta-value">{{ formatDateTime(detail.staffOrderTime) }}</span>
                    </div>
                </div>
                <div class="row-item">
                    <div class="meta-item">
                        <span class="meta-label">入库单号:</span>
                        <span class="meta-value">{{ detail.ruKuNo || "—" }}</span>
                    </div>
                    <div class="meta-item" v-if="detail.pdfUrl">
                        <span class="meta-label">合同/附件:</span>
                        <a class="meta-link" :href="detail.pdfUrl" target="_blank" rel="noopener">查看</a>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="section-block">
            ...
            <div class="table-box">
                <el-table :data="productList" header-cell-class-name="table-header-cell"
...
export default {
    ...
    methods: {
        ...
        isPayText(v) {
            if (v === 1 || v === "1") return "已付款";
            if (v === 0 || v === "0") return "未付款";
            ...
            this.$api({ url: DETAIL_API, method: "post", data: { id } })
                .then(res => {
                    ...
                    this.detail = {
                        ...
                        isPay: d.isPay,
                        staffOrderNo: d.staffOrderNo ?? "",
                        staffOrderTime: d.staffOrderTime ?? "",
                        ruKuNo: d.ruKuNo ?? "",
                        pdfUrl: d.pdfUrl ?? ""
                    };
                    ...
                    this.productList = rows.map(it => {
                        ...
                        return {
                            id: it && it.foreignProductId != null ? String(it.foreignProductId) : (info.id != null ? String(info.id) : ""),
