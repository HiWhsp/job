<template>
  <div class="view-wrap add-purchase-page">
    <h1 class="page-title">新增其他采购单</h1>
    ...
    <div class="product-section">
      <div class="section-header">
        ...
        <div class="section-actions">
          ...
          <el-button type="primary" @click="handleAddProduct">添加采购产品</el-button>
        </div>
      </div>
      <div class="table-box">
        <el-table
        ...
        >
          ...
          <el-table-column label="采购产品" min-width="140">
...
export default {
  ...
  computed: {
    purchaseTotal() {
      return this.productList.reduce((sum, row) => {
  ...
  methods: {
    ...
    handleSaveRow(index) {
      const row = this.productList[index];
      if (!row.productName || !row.unitPrice || !row.quantity) {
        this.$message.warning("请填写采购产品、单价、数量");
        return;
      }
    ...
    handleEditRow(index) {
      const row = this.productList[index];
      const hasEditing = this.productList.some(r => r.isEditing);
      if (hasEditing) {
    ...
    handleBatchDelete() {
      if (this.selectedRows.length === 0) {
        this.$message.warning("请先勾选要删除的采购产品");
        return;
      }
    ...
    handleSubmit() {
      this.$refs.baseForm.validate(valid => {
        ...
        if (hasEditing) {
          this.$message.warning("请先保存正在编辑的采购产品行");
          return;
        }
