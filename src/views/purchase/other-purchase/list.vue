          ...
          <el-table-column label="操作" align="left">
            <template slot-scope="{ row }">
              <span class="row-acts">
                ...
                <span v-if="row.status === '待财务付款' || row.status === '采购中'" class="row-act"
                  @click="handleComplete(row)">采购完成</span>
              </span>
            </template>
          </el-table-column>
        </el-table>
...
<script>
const LIST_API = "/getPurchaseOtherOrderList";
const FINISH_API = "/finishPurchaseOtherOrder";
const DELETE_API = "/delPurchaseOtherOrder";
...
export default {
  ...
  methods: {
    ...
    handleEdit(row) {
      ...
      this.$router.push({ name: "other-purchase-add", query: { id } });
    },
    handleDelete(row) {
