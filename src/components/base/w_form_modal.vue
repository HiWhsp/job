<template>
	<div class="role-form-modal">
			...
			<div class="modal-ctx" data-type="基础表单弹窗">
				<div class="input-items">
                    ...
                    ref="formSelect" /> -->
									...
									<!-- 时间选择 -->
									<!-- <el-time-picker v-else-if="item.form_render_type == 'time'"
										:disabled="form_field_disabled_map[item.field]" value-format="HH:ss"
										v-model="form[item.field]" placeholder="请选择时间">
									</el-time-picker> -->
									...
									<div v-else-if="item.form_render_type == 'cascader'" type="级联选择"
										class="cascader-box">
										<el-cascader v-model="form[item.field]" :options="cascaderOptions" :props="{
											emitPath: false,
											children: 'child',
											value: 'id',
											label: 'name',
									...
									<div v-else-if="item.form_render_type == 'treeSelectForProductCategory'"
										type="分类树选择" tip="适用于树形分类选择" class="tree-box tree-box-cate">
										<el-popover ref="popoverTree" placement="top-start" title=""
											:width="treePopWidth" trigger="click" content="">
											<div slot="reference" class="tree-click-box">
		...
		<el-dialog :visible.sync="videoShow">
			<video width="100%" :src="form.videoUrl" controls alt="" />
		</el-dialog>
	</div>
</template>
	...
	export default {
		...
		methods: {
			...
			query_detail(row) {
				...
				this.$api({
					...
					if (res.code == 200) {
						...
						if (info.parentIdArr) {
							try {
								let checked_list = JSON.parse(info.parentIdArr)
								this.$nextTick(() => {
									this.$refs['product_cate_tree_channelId'][0].setCheckedNodes(
										checked_list);
								})
							} catch (e) {
								//TODO handle the exception
							}
							form.parentIdArr = info.parentIdArr
						} else {
							form.parentIdArr = ''
						}
			...
			handle_upload_image_request(uploadInfo, formItem) {
				...
				this.$apiUpload(formData).then((res) => {
					console.warn("apiUpload图片上传res", res);
					let {
						type,
						saveFormat,
						col
			...
			handle_upload_video_request(uploadInfo, formItem) {
				...
				this.$apiUploadVideo({
					...
					// console.warn("apiUpload 视频上传res", res);
					if (res.code == 200) {
						let url = res.url;
						this.form[formItem.field] = url;
						console.log(this.form, formItem.field);
					}
				});
			},
			...
			tree_handleCheckChange(data, checked, indeterminate, form_item) {
				...
				if (this.is_finish) {
					...
					this.form['parentIdArr'] = JSON.stringify(tree_cates.map(v => ({
						id: v.id,
						title: v.title,
					})))
