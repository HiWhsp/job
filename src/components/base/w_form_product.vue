					...
					<div class="input-box">
						...
						<div class="val">
							<div class="form-ele">
								...
								<!-- 多行文本 -->
								<el-input clearable="" v-else-if="item.form_render_type == 'textarea'"
									:disabled="form_field_disabled_map[item.field]" v-model="form[item.field]"
								...
								<div class="select-box" v-else-if="item.form_render_type == 'select'">
									<template v-if="select_config_map[item.field]">
										<el-select clearable="" :disabled="form_field_disabled_map[item.field]"
											...
											<el-option
												v-for="(select_item, select_index) in select_config_map[item.field].options"
			            ...
			            >
								...
								<div type="视频上传" class="video-upload-box"
									...
									<el-upload class="upload-video" list-type="text" accept="video/*"
										:multiple="item.upload_multiple" :limit="item.upload_limit" :name="upload_name"
										:action="mix_upload_action" :data="upload_data"
								...
								<div v-else-if="item.form_render_type == 'treeSelect'" type="分类树选择" tip="适用于树形分类选择"
									class="tree-box tree-box-brand">
									<el-popover ref="popoverTree" placement="top-start" title="" :width="treePopWidth"
								...
								<div v-else-if="item.form_render_type == 'treeSelectForProductCategory'" type="分类树选择"
									tip="适用于树形分类选择" class="tree-box tree-box-cate">
									<el-popover ref="popoverTree" placement="top-start" title="" :width="treePopWidth"
										trigger="click" content="">
										<div slot="reference" class="tree-click-box">
										...
										<div class="pop-tree" v-if="tree_config_map[item.field]">
											...
											<el-tree :ref="'product_cate_tree_' + item.field"
												:data="tree_config_map[item.field].treeData"
												:props="tree_config_map[item.field].treeProps" :show-checkbox="true"
												:node-key="tree_config_map[item.field].treeNodeKey"
												:default-expand-all="tree_config_map[item.field].defaultExpandAll"
												:expand-on-click-node="false" :highlight-current="true"
												@check-change="(data, checked, indeterminate) => tree_handleCheckChange(data, checked, indeterminate, item)"
...
export default {
	...
	methods: {
		...
		initForm(row = {}) {
			console.log('w_form_product 表单数据回显', row)
			...
			this.$log('初始化 tree_field_title_map', this.tree_field_title_map)
		},
		...
		render_detail(data) {
			...
			//特殊处理-商品分类多选
			if (data.channelIdArr) {
				try {
					let checked_list = JSON.parse(data.channelIdArr)
					this.$nextTick(() => {
						this.$refs['product_cate_tree_channelId'][0].setCheckedNodes(checked_list);
					})
				} catch (e) {
					//TODO handle the exception
				}
				form.channelIdArr = data.channelIdArr
			} else {
				form.channelIdArr = ''
			}
			...
			this.$log('form_product 表单回显 form_fields', this.form_fields)
			this.$log('form_product 表单回显 this.form', this.form)
		...
		tree_handleCheckChange(data, checked, indeterminate, form_item) {
			...
			if (this.is_finish) {
				...
				// this.$log('form_item', form_item);


				let field = form_item.field
				let cates = this.$refs['product_cate_tree_' + field][0].getCheckedNodes();
				let keys = this.$refs['product_cate_tree_' + field][0].getCheckedKeys()
				console.log('获取选择的节点分类数据 cates', cates)
				// console.log('获取选择的节点 keys', keys)


				const tree_cates = cates.filter(v => v.child.length === 0);

				this.form['channelIdArr'] = JSON.stringify(tree_cates.map(v => ({
					id: v.id,
					title: v.title,
				})))
				this.form[field] = tree_cates.map(v => v.id).join(',');
				this.tree_field_title_map[field] = tree_cates.map(v => v.title).join(',');
				// this.$refs["popoverTree"][0].doClose();
				// console.warn("弹窗", );
				this.$log('tree_field_title_map', this.tree_field_title_map)
			}
