<template>
	<div class="custom-form-modal">
		<el-dialog :title="title" width="600px" custom-class="modal-form" :close-on-click-modal="false"
			:close-on-press-escape="false" :visible.sync="show_modal" :append-to-body="true"
...
export default {
	...
	methods: {
		...
		do_download() {
			this.$apiDownload({
				url: '/medicine/goods/importTemplate',
				method: 'post',
				data: {
				}
			}).then(res => {
				...
				link.href = url;
				link.setAttribute('download', '产品导入模版.xlsx'); // 指定文件名
