...
const marketingRoutes = [
  {
    ...
    children: [
      ...
      // 订单审核详情
      {
        ...
        component: () =>
          import("@/views/marketing/order-audit-list/detail.vue"),
        meta: {
          title: "订单审核详情",
          requireAuth: true,
          hidden: false,
...
const productionVicePresidentRoutes = [
  {
    ...
    children: [
      ...
      },
      {
        ...
        component: () =>
          import(
            "@/views/production-vice-president/material-purchase/detail.vue"
          ),
        meta: {
          title: "原料采购详情",
          requireAuth: true,
      ...
      // 外采包装
      {
        ...
        component: () =>
          import(
            "@/views/production-vice-president/external-package-purchase/index.vue"
          ),
        meta: {
          title: "外采包装",
          requireAuth: true,
      ...
      },
      {
        ...
        component: () =>
          import(
            "@/views/production-vice-president/external-package-purchase/detail.vue"
          ),
        meta: {
          title: "外采包装采购详情",
          requireAuth: true,
      ...
      },
      {
        ...
        component: () =>
          import(
            "@/views/production-vice-president/device-purchase/detail.vue"
          ),
        meta: {
          title: "设备采购详情",
          requireAuth: true,
...
/** 管理员端：不注册业务员「新增订单」路由，菜单与直链均不可进 */
function stripAdministratorSalesOrderAddRoute(routes) {
  const walk = (nodes) => {
    if (!nodes || !nodes.length) return;
    for (const node of nodes) {
      if (node.children && node.children.length) {
        if (node.name === "r9s-order") {
          node.children = node.children.filter(c => c.name !== "r9s-order-add");
        }
        walk(node.children);
      }
    }
...
/** 管理员合并路由下，指定页面仅展示数据（列表/详情只读，由页面读 meta） */
const ADMINISTRATOR_READ_ONLY_ROUTE_NAMES = new Set([
  "r9s-order-list",
  "r9s-order-detail",
  "r9wh-product-inventory-management-list",
  "r9whm-material-warehouse-management-list"
]);

function markAdministratorReadOnlyRouteMeta(routes) {
  const walk = (nodes) => {
    if (!nodes || !nodes.length) return;
    for (const node of nodes) {
      if (node.name && ADMINISTRATOR_READ_ONLY_ROUTE_NAMES.has(node.name)) {
        node.meta = { ...(node.meta || {}), administratorReadOnly: true };
      }
...
const administratorRoutes = (() => {
  ...
  stripAdministratorSalesOrderAddRoute(routes);
  markAdministratorReadOnlyRouteMeta(routes);
  return routes;
})();
